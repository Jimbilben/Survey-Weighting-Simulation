...
output_summary_cat <- regression_output_cat %>% 
  group_by(type, bias) %>% 
...
regression_output_wide_cat <- regression_output_cat %>% 
...
#data_test2 <- data_test  tibble(outcome = sample(c("a", "a", "a", "b", "b", "c"), 1000, TRUE))
